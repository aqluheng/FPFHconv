...
from modelbak import DGCNN_partseg
import numpy as np
...
def MyVisualization(visu, visu_format, data, pred, seg, label, partseg_colors, class_choice):
    global class_indexs
    global visual_warning
    visu = visu.split('_')
    i = 0
    ...
    FPFHHist = torch.zeros(1,33,2048)
    for batchId in range(1):
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(data[batchId].cpu().transpose(0,1))
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(20))
        pfph = o3d.registration.compute_fpfh_feature(pcd, search_param=o3d.geometry.KDTreeSearchParamKNN(20))
        FPFHHist[batchId] = torch.Tensor(pfph.data)
    FPFHHist = FPFHHist.cuda()
    FPFHneighor = knn(FPFHHist, k=20)[0]

    highlightRange = range(0)


    for idx in highlightRange:
        # for pointIdx in FPFHneighor[idx]:
        #     RGB[pointIdx] = [0,255,0]
        RGB[idx] = [255,0,0]

    for pointIdx in FPFHneighor[50]:
        RGB[pointIdx] = [0,255,0]


    for pointIdx in FPFHneighor[100]:
        RGB[pointIdx] = [255,0,0]


    for pointIdx in FPFHneighor[150]:
        RGB[pointIdx] = [0,0,255]

    pred_np = []
    pred_np.append(pred[0].cpu().numpy())
    xyz_np = data[0].cpu().numpy()
...
def test(args, io):
    ...
    for data, label, seg in test_loader:
        ...
        test_label_seg.append(label.reshape(-1))
        # visiualization
        MyVisualization(args.visu, args.visu_format, data, pred, seg, label, partseg_colors, args.class_choice) 
        exit()
